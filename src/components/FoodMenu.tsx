...
import { foodMenu, menuCategories } from "@/data/foodMenu";
...
function MenuItemCard({ item, index }: { item: MenuItem; index: number }) {
  return (
    ...
    >
      ...
      {/* Options (like Naija Style / Lebanese Style) */}
      {item.options && (
        <div className="space-y-2 mt-3 border-t border-[var(--gold)]/10 pt-3">
          {item.options.map((opt, i) => (
            <div key={i} className="flex justify-between items-start gap-2">
              ...
              <span className="text-[var(--gold)] text-xs font-medium whitespace-nowrap">{formatPrice(opt.price)}</span>
            </div>
          ))}
        </div>
      )}
...
export default function FoodMenu() {
  ...
  return (
    <section id="menu" className="relative py-20 md:py-32 px-4 bg-[var(--warm-black)]">
      {/* Background pattern */}
      ...
      <div className="relative max-w-7xl mx-auto" ref={ref}>
        ...
        >
          ...
          <p className="text-[var(--cream)]/60 max-w-xl mx-auto text-sm">
            From hearty breakfasts to exquisite dinners, explore our carefully curated selection
          </p>
        </motion.div>
              ...
              >
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {category.items.map((item, itemIndex) => {
                    ...
                    // Handle sub-items (e.g., category with sub-names)
                    return (
                      <div key={item.name}>
                        <h4 className="font-serif text-[var(--cream)] mb-2">{item.name}</h4>
                      </div>
                    );
                  })}
                </div>
